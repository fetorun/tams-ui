...
import { errorAndExit, getWorkspacePackages } from '@tams-ui/build-utils'
...
async function main() {
  ...
  consola.debug(chalk.yellow(`Updating package.json for tams-ui`))

  const pkgs = Object.fromEntries(
    (await getWorkspacePackages()).map((pkg) => [pkg.manifest.name!, pkg])
  )
  const elementPlus = pkgs['tams-ui'] || pkgs['@tams-ui/nightly']
  const eslintConfig = pkgs['@tams-ui/eslint-config']
  const metadata = pkgs['@tams-ui/metadata']
